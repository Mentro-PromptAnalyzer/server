...
The Compose project is `mentro-server-local`. Its API service has DNS name `mentro-server` on `mentro-server-local_pilot` and listens internally on port 3001. The fixture service is `fixture:3004` on that internal network and also joins the preview network with a loopback-only host port at `127.0.0.1:3004` for browser contract tests. The API also joins `mentro-server-local_preview` so `127.0.0.1:3001` can reach it; the host binding does not expose it to the LAN. Another isolated Compose project can explicitly join the `mentro-server-local_pilot` network to test a real container-to-container call. Do not use this fixture network or token in production.
...
The WebApp at `http://127.0.0.1:8081` can sign in with `preview@example.test` / `preview-only` (or `second@example.test` with the same disposable password). Only this origin receives fixture CORS responses. Sessions expire after one hour; logout revokes the session. Restarting the fixture loses all sessions and in-memory history. There is no refresh-token or OAuth implementation: sign in again for a new session.

This is a browser-contract substitute, **not Supabase Auth/RLS/persistence evidence**. It permits a real WebApp-to-Mentro streamed reply (`Hello world`), history UI exercise, and explicit invalid-session/user-separation checks against the fixture. The production API's authentication path is unchanged; it still checks the configured upstream. No fixture source is copied into the production API image. Never add production data or credentials to this preview.

`tests/container/browser-session.test.mjs` verifies the HTTP session/real-server stream contract, unapproved-origin denial, invalid login, isolated fixture records, and logout revocation. These local results cannot close M5 or prove hosted provider behavior.
